...
class Wire {
  ...
  async _httpTrigger(channel, event, data) {
    ...
    const path = "/apps/".concat(this._appId, "/events");
    ...
    if (!response.ok) {
      ...
      throw new Error("Wire trigger failed [".concat(response.status, "]: ").concat(text));
    }
    return response.json().catch(() => ({}));
  }
  async _md5(text) {
    try {
      const _await$import = await import('crypto'),
        createHash = _await$import.createHash;
      return createHash('md5').update(text).digest('hex');
    } catch (_unused) {
      throw new Error('Wire server-side triggers require Node.js (built-in crypto module).');
    }
  }
  async _hmacSha256(secret, data) {
    try {
      const _await$import2 = await import('crypto'),
        createHmac = _await$import2.createHmac;
