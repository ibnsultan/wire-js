...
class Wire {
  ...
  async trigger(channel, event, data = {}) {
    ...
    if (!ch) {
      throw new Error(`Wire: not subscribed to "${channel}". Call wire.subscribe() first.`);
    }
  ...
  async channelInfo(channel, attributes = []) {
    ...
    return this._httpGet(`/apps/${this._appId}/channels/${encodeURIComponent(channel)}`, attributes.length ? {
  ...
  async triggerBatch(events) {
    ...
    return this._httpPost(`/apps/${this._appId}/batch_events`, {
      batch
    });
  }
  ...
  _requireMode(expected, methodName) {
    if (this._mode !== expected) {
      throw new Error(`Wire.${methodName}() is only available in ${expected} mode.`);
    }
  }
